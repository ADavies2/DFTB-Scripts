        ...
        if [[ $size2 > $size ]]; then
          ...
          if grep -q "Geometry converged" detailed.out && grep -q "Geometry converged" $3.log; then
            if [[ $7 == '1e-5' ]]; then
              ...
              rm $3.out *log *xyz *gen *bin submit_$3 *dat *xml
              RESULT='success1'
              STALL='none'
              break
...
!
            ...
            echo "$3 did NOT converge. User trouble-shoot required to check atoms."
            exit
        ...
        if [[ $size2 > $size ]]; then
          ...
          if grep -q "Geometry converged" detailed.out && grep -q "Geometry converged" $3.log; then
            if [[ $7 == '1e-5' ]]; then
              ...
              cp charges.dat "1e-4-Outputs/$COF-charges.dat"
              rm $3.out *log *xyz *gen charges* submit_$3 *bin *xml
              RESULT='success1'
              STALL='none'
              break
